...
resp, body = update_event({
  "@context": "http://www.markus-lanthaler.com/hydra/event-api/contexts/Event.jsonld",
  "@type": "Event",
  "@id": str(evt.identifier),
  "name": str(evt.value(SCHEMA.name)),
  "description": str(evt.value(SCHEMA.description)),
  "start_date": str(evt.value(SCHEMA.startDate)),
  "end_date": str(evt.value(SCHEMA.endDate)),
})
if resp.status // 100 != 2:
    print("failed... (%s %s)" % (resp.status, resp.reason))
else:
